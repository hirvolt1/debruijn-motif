...
pam30 = filter(lambda x: x[0] in cols, pam30)
for col in cols:
    if col not in map(lambda x: x[0], pam30):
        pam30.append([col] + [0]*(len(cols)-1))
pam30 = sorted(pam30, key=lambda x: x[0])

pam30 = map(lambda x: list(x), zip(*pam30)) # transpose
pam30 = filter(lambda x: x[0] in cols, pam30)
for col in cols:
    if col not in map(lambda x: x[0], pam30):
        pam30.append([col] + [0]*(len(cols)-1))
...
print "\n".join(map(lambda x: str(list(x)), pam30))
print "const int pam30[{0}][{0}] = {{".format(len(cols)-1)

arr = [x[1:] for x in pam30[1:]]
for line in arr:
    print "    {" + ", ".join(map(lambda x: str(x).rjust(3), line)) + "},"
print "};"
